...
internal fun ErdBoardViewModel.handleCanvasIntent(boardIntent: ErdBoardIntent) = intent {
    when (boardIntent) {
        is ErdBoardIntent.OnPanZoom -> reduce {
            val newScale = (state.scale * boardIntent.zoom).coerceIn(ErdBoardConfig.MIN_SCALE, ErdBoardConfig.MAX_SCALE)
            val factor = newScale / state.scale
            state.copy(
                scale = newScale,
                panOffset = ErdBoardVector(
        ...
        is ErdBoardIntent.OnSetViewTransform -> reduce {
            state.copy(
                scale = boardIntent.scale.coerceIn(ErdBoardConfig.MIN_SCALE, ErdBoardConfig.MAX_SCALE),
        ...
        is ErdBoardIntent.OnDragStart -> {
            ...
            runtimeState = runtimeState.clearDragPreviewThrottle()
            reduce { state.copy(draggingNodeIds = state.draggingNodeIds + boardIntent.nodeId) }
        ...
        is ErdBoardIntent.OnDragNode -> {
            ...
            sendNodeDragPreview(boardIntent.nodeId, newPosition)
        }
        ...
        is ErdBoardIntent.OnDragEnd -> {
            ...
            reduce { state.copy(draggingNodeIds = state.draggingNodeIds - nodeId) }
            if (origin != null && currentPosition != null && origin != currentPosition) {
                ...
                commitNodeMoveAndReleaseLock(nodeId, currentPosition)
            } else {
        ...
        is ErdBoardIntent.OnMultiDragEnd -> {
            runtimeState = runtimeState.clearDragPreviewThrottle()
            reduce { state.copy(draggingNodeIds = state.draggingNodeIds - boardIntent.nodeIds.toSet()) }
            ...
            if (movedPairs.isNotEmpty()) {
                ...
                reduce { state.copy(canUndo = runtimeState.canUndo) }
                commitNodeMovesAndReleaseLocks(
